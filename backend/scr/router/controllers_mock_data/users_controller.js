...
// 🎯 全域 Mock Data 資料庫
let user_list = [
    { "id": 1, "user_name": "Tom", "password": "Tom" },
    { "id": 2, "user_name": "Admin", "password": "admin" }
];
...
async function register (req, res) {
    ...
    try {
        // 檢查帳號是否重複
        const isDuplicate = user_list.some(u => u.user_name === name);
        if (isDuplicate) {
        ...
        return successMessage(res, 201, "用戶建立成功", newUser);
    } catch (error) {
...
async function login (req, res) {
    ...
    try {
        // 使用 .find() 尋找符合帳密的用戶
        const exist_user = user_list.find(u => u.user_name === name && u.password === pwd);
...
async function deleteUser (req, res) {
    try {
        const userId = parseInt(req.params.id);
        const userExists = user_list.some(u => u.id === userId);
        ...
        // 從模擬資料庫中過濾掉
        user_list = user_list.filter(u => u.id !== userId);
...
async function updatePassword (req, res) {
    ...
    try {
        ...
        if (user_list[userIndex].password != old_pwd) {
            return errorMessage(res, 404, `舊密碼錯誤`)
        }

        // 修改密碼
        user_list[userIndex].password = new_pwd;
        return successMessage(res, 200, "用戶更改密碼成功", `new password: ${new_pwd}`);
    } catch (error) {
